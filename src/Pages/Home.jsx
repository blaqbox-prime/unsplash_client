...
import { anim, fadeIn, widen } from '../Utils/animations'

function Home() {


  return (
    <main className='page w-full dark:bg-primary'>
      <motion.img
      {...anim(fadeIn)}
      ...
      src={assets.images.hero} alt="background" className='w-full h-full object-cover pointer-events-none -z-10'/>
       ...
       className='absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] flex flex-col items-center gap-5 transition justify-center w-full text-center max-w-md md:max-w-lg'>
            <h1 className='text-4xl font-bold dark:text-light'>Search</h1>
            <p className='text-secondary dark:text-fadedLight'>Search high-resolution images from Unsplash</p>
            <motion.div 
            {...anim(widen)}
